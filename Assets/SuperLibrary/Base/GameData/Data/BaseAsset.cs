...
{
    [Header("Cost Coin")]
    public int unlockCoinBase = 100;
    public float unlockCoinScale = 1;
    public int unlockCoinMax = 300;

    [Header("Cost Ads")]
    public int unlockAdsBase = 3;
    public float unlockAdsScale = 1;
    public int unlockAdsMax = 5;

    [Header("Cost Level")]
    public int unlockLevelBase = 3;
    public float unlockLevelScale = 3;

    [Header("Cost Star")]
    public int unlockStarBase = 1;
    public float unlockStarScale = 0.5f;
    ...
    public T Current
    {
        get
        {
            ...
            {
                current = list.FirstOrDefault(x => x.isSelected && x.isUnlocked);
            }
    ...
    public void ConvertToData(List<StageData> saveData)
    {
        foreach (var i in saveData)
        {
            try
    ...
    public void UpdateCost()
    {
        ...
        for (int i = 0; i < tempList.Count; i++)
        {
            ...
            tempList[i].unlockType = UnlockType.Star;
        }
        ...
        for (int i = 0; i < tempList.Count; i++)
        {
            ...
            {
                item.unlockPrice = Mathf.Clamp(Mathf.FloorToInt(unlockCoinBase + unlockCoinBase * coinIndex * unlockCoinScale), unlockCoinBase, unlockCoinMax);
                coinIndex ++;
                Debug.Log(item.index + " " + item.unlockType + " " + item.unlockPrice + " " + item.id);
            }
            else if (item.unlockType == UnlockType.Ads)
            {
                item.unlockPrice = Mathf.Clamp(Mathf.FloorToInt(unlockAdsBase + unlockAdsBase * adsIndex * unlockAdsScale), unlockAdsBase, unlockAdsMax);
                adsIndex ++;
                Debug.Log(item.index + " " + item.unlockType + " " + item.unlockPrice + " " + item.id);
            }
            else
            {
                if(item.unlockType == UnlockType.Star)
                {
                    if(i < 1)
                        tempList[i].isUnlocked = true;
                }
                item.unlockPrice = Mathf.RoundToInt(unlockStarBase + unlockStarScale) * i;
                //starIndex += i;
                Debug.Log(item.index + " " + item.unlockType + " " + item.unlockPrice + " " + item.id);
            }
        }
...
public class CloudStages
{
    ...
    public static StageData UpdateToStageData(StageData stageData, StageCloud i)
    {
        ...
        stageData.unlockType = i.unlockType;
        return stageData;
    }
}