...
public class FirebaseHelper : MonoBehaviour
{
    ...
    //public static FirebaseMessage firebaseMessage = null;
    //private static FirebaseNotification notification = null;
#endif
        ...
        if (DependencyStatus != DependencyStatus.Available && Status != FirebaseStatus.Checking)
        {
            ...
            while (Status == FirebaseStatus.Checking && elapsedTime < timeOut && isProcess)
            {
                if (DebugMode.IsDebugMode)
                    Debug.LogError("[Firebase] DoCheckStatus: " + Status + " " + elapsedTime.ToString("0.0"));
...
//    }
    ...
    public static void SetUserData(UserData user)
    {
        if (user != null)
        {
            if (DataManager.CurrentItem != null)
                SetUser("Item", DataManager.CurrentItem.id);
        }
    }
...
#if EXISTED_FIREBASE_MESSAGING
    private static IEnumerator WaitServerOnlineToUpdate(string token)
    {
        yield return new WaitUntil(() => NetManager.Instance.IsOnline());
        AccountManager.Instance.FcmToken = token;
        SetUser("Code", AccountManager.Instance.Code.ToString());
    }
#endif
}
...
public enum AdAmaEvent
{
    fullads_request,
    fullads_request_failed,
    fullads_request_success,
    fullads_show_ready,
    fullads_show_notready,
    fullads_show_notinternet,
    fullads_show,
    fullads_show_failed,
    fullads_finish,
    fullads_click,
    ...
    videoads_request,
    videoads_request_failed,
    videoads_request_success,
    videoads_show_ready,
    videoads_show_notready,
    videoads_show_notinternet,
    videoads_show,
    videoads_show_failed,
    videoads_finish,
    videoads_click
}